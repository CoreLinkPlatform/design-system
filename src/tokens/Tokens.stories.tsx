...
const radii = [['sm', coreTokens.radius.sm], ['md', coreTokens.radius.md], ['lg', coreTokens.radius.lg]] as const;
const swatches = [
...
export const CanonicalBrandAndShape: Story = { render: () => <div className="core-root" dir="ltr" lang="en" style={{ display: 'grid', gap: 24, padding: 24 }}>
  ...
  <section><h2>Radius: 4 / 8 / 12px</h2><div style={{ display: 'flex', flexWrap: 'wrap', gap: 16 }}>{radii.map(([name, value]) => <div key={name} style={{ inlineSize: 120, blockSize: 72, display: 'grid', placeItems: 'center', border: '1px solid var(--core-color-border-strong)', borderRadius: value }}>{name}: {value}</div>)}</div></section>
</div> };
